...
$(function () {
    //截取地址的？问号后的字符串
     ...
     tk = tk != undefined ? tk : '';
	 tk="04367748-afd2-4c73-ae96-d75c4c320536"
     var header = {
     ...
     $.ajax({
         type: "POST",
          data: "",
          url: 'http://10.20.19.209/seeyon/rest/oa3/revert/queryLoginUserInfo',
          async: false,
          headers: header,
          dataType: "html",
    ...
    $(".meeting-ioc").click(function () {
        console.log("跳转成功");
        window.location.href = "meeting_rooms_content.html";
    });
    ...
    var futureDate = [];
    // var futureDate = ["2019-09-02", "2019-09-09", "2019-09-01", "2019-02-02", "2019-10-02"];
    var alikeDate = [];

    for (var i = 0; i < 5; i++) {
        var milliseconds = date.getTime() + 1000 * 60 * 60 * 24 * i; //当i为0代表当前日期，为1时可以得到明天的日期，以此类推
        ...
        futureDate.push(year + '-' + month + '-' + day);
    }
    console.log(futureDate);
    for (var i = 0; i < futureDate.length; i++) {
        for (var j = 0; j < legalHoliday.length; j++) {
            if (futureDate[i] == legalHoliday[j]) {
                alikeDate.push(futureDate[i]);
            }
        }
    }
    var seven = [];
    console.log("总数", alikeDate.length)
    if (alikeDate.length >= 2) {
        for (var i = 0; i < 7; i++) {
            var milliseconds = date.getTime() + 1000 * 60 * 60 * 24 * i; //当i为0代表当前日期，为1时可以得到明天的日期，以此类推
            var newMyDate = new Date(milliseconds);
            var year = newMyDate.getFullYear();//获取当前年
            ...
            seven.push(year + '-' + month + '-' + day);
        }
        future = seven.pop() + " 23:59";
        localStorage.setItem("future", future)
        console.log("7", future)
    } else {
        future = futureDate.pop() + " 23:59";
        localStorage.setItem("future", future)
        console.log("5", future)
    }
    $(".form_date2").datetimepicker('setEndDate', new Date(document.getElementById("admin-date").value = future));
    $(".form_date3").datetimepicker('setEndDate', new Date(document.getElementById("admin-date").value = future));
...
function login() {
    ...
    if (applyName != null) {
        for (var a = 0; a < applyName.length; a++) {
            name = document.getElementById("input-apply-name").value = applyName[a].name;
            ...
            loginName = applyName[a].loginName;
        ...
        $.ajax({
            //A108248黄浩彬
            // url: (url + "/seeyon/rest/orgMember/code/"+code+"?token=" + token),
            ...
            success: function (data) {
                ...
                document.getElementById("input-apply-name").value = dateOrgmeber.name;
                document.getElementById("hidden-apply-id").value = dateOrgmeber.id;
                document.getElementById("hidden-DepartmentId").value = dateOrgmeber.orgDepartmentId;
                //document.getElementById("input-apply-hone").value = dateOrgmeber.telNumber;
        ...
        $.ajax({
            ...
            // url: (url + "/seeyon/rest/orgMember/code/"+code+"?token=" + token),
            // url: (url + "/seeyon/rest/orgMember?loginName="+loginName+"&token=" + token),
            url: (url + "/seeyon/rest/orgMember?loginName=liuquan&token=" + token),
            type: "GET",
            dataType: "text",
            async: false,
            processData: false,
            success: function (data) {
                ...
                document.getElementById("hidden-apply-id").value = dateOrgmeber.id;
                document.getElementById("hidden-DepartmentId").value = dateOrgmeber.orgDepartmentId;
                var telNumber = dateOrgmeber.telNumber;
                loginName = dateOrgmeber.loginName;
                localStorage.setItem("loginName", loginName);
...
function save() {
    ...
    if (applyName != null) {
        for (var a = 0; a < applyName.length; a++) {
            ...
            loginName = localStorage.getItem("loginName");
            console.log(applyName[a].telNumber)
...
function submit() {
    ...
    var futureTime = localStorage.getItem('future');
    var futureDa = futureTime.substring(0, 10);
    if (Theme == "") {
        hint("请填写主题");
        return false;
    } else if (ApplyHone == "") {
        ...
        return false;
    } else if (startTime > futureDa) {
        hint("目前仅限申请" + futureTime + "之前的时间");
        return false;
    } else if (endTime > futureDa) {
        hint("目前仅限申请" + futureTime + "之前的时间");
        return false;
    }

    var str = "<formExport version=\"2.0\" ><summary id=\"-1\"  name=\"formmain_0015\" /><values>   <column name=\"申请人\" ><value>" + applyId + "</value></column><column name=\"申请日期\" ><value>" + applyDateTime + "</value></column><column name=\"会议主题\" ><value>" + Theme + "</value></column><column name=\"申请部门\" ><value>" + DepartmentId + "</value></column><column name=\"会议开始时间\" ><value>" + dateStart + "</value></column><column name=\"是否有外单位参加\" ><value>" + WaiQi + "</value></column><column name=\"会议组织者\" ><value>" + TextureId + "</value></column><column name=\"申请人联系电话\" ><value>" + ApplyHone + "</value></column><column name=\"会议出席人数\" ><value>" + ChuXi + "</value></column><column name=\"备注\" ><value>" + remarkTxt + "</value></column>          <column name=\"会议截止时间\" ><value>" + dateEnd + "</value></column><column name=\"参会人员\" ><value>" + renyuanTxtId + "</value></column><column name=\"会议记录者\" ><value>" + JiLuId + "</value></column><column name=\"会议类型\" ><value>" + typeSelect + "</value></column><column name=\"会议主持者\" ><value>" + DirectId + "</value></column><column name=\"议程议题\"><value>" + yitiTxt + "</value></column><column name=\"会议实际结束时间\" ><value/></column><column name=\"会议室所在地\" ><value>956163389618735506</value></column><column name=\"L1层领导参会\" ><value>" + KongGu + "</value></column><column name=\"是否邀请L1领导参会\" ><value>" + KongGu + "</value></column><column name=\"会议形式\" ><value>" + xinshiSelect + "</value></column></values></formExport>";
    var token = getToken("rest", "123456");
    var url = "http://10.1.19.170";
    loginName = localStorage.getItem("loginName");
    console.log("发起人:", loginName)
    var jsonData = {
    ...
    console.log(str);
    $.ajax({
        url: (url + "/seeyon/rest/flow/MeetingApply?token=" + token),
        type: "POST",
        dataType: "json",
        async: false,
...
function sel(value) {
    ...
    if (value == 0) {
        ...
        ShenHe += " <label class=\"theme-label\">";
        ShenHe += " 控股领导会议审核人:";
        ShenHe += "</label>";
        ShenHe += "</div>";
        ShenHe += "";
...
function TabTwo() {
    ...
    if (flag) {
        ...
        $.ajax({
            url: (url + "/seeyon/rest/coll/sentAffairs?token=" + id),
            type: "POST",
            dataType: "text",
            contentType: "application/json",
            async: false,
...
function summary(summayId) {
    ...
    var subMenu = "";
    $.ajax({
        url: (url + "/seeyon/rest/meeting/colSummary/" + sid + "/?token=" + token),
        type: "GET",
        dataType: "JSON",
        contentType: "application/json",
        async: false,
        ...
        success: function (date) {
            if (date == "") {
                $.ajax({
                    url: (url + "/seeyon/rest/meeting/activity/" + sid + "/?token=" + token),
                    type: "GET",
                    dataType: "JSON",
                    contentType: "application/json",
                    async: false,
...
// //选择人员
// function Personnel(str) {
//     var winObj = window.open('dist/index.html');
//     winObj.onload = function () {
//         //     // unload事件放在load事件中是因为直接监听unload事件，页面在打开时会直接触发一次unload事件
//         winObj.onunload = function () {
//
//             var localPersonnel = localStorage.getItem('Personnel');
//             var Personnel = eval('(' + localPersonnel + ')');
//             console.log("选中了", Personnel);
//             Personnel.forEach(function (value, index) {
//                 if (str == 'applyName') {
//                     loginName = Personnel[index].loginName;
//                     document.getElementById("input-apply-name").value = Personnel[index].name;
//                     document.getElementById("hidden-apply-id").value = Personnel[index].id;
//                     document.getElementById("hidden-DepartmentId").value = Personnel[index].orgDepartmentId;
//                     document.getElementById("input-apply-hone").value = Personnel[index].telNumber;
//                     return false;
//                 } else if (str == 'texture') {
//                     document.getElementById("input-texture").value = Personnel[index].name;
//                     document.getElementById("hidden-texture-id").value = Personnel[index].id;
//                     return false;
//                 } else if (str == 'direct') {
//                     document.getElementById("input-direct").value = Personnel[index].name;
//                     document.getElementById("hidden-direct-id").value = Personnel[index].id;
//                     return false;
//                 } else if (str == 'jilu') {
//                     document.getElementById("input-jilu").value = Personnel[index].name;
//                     document.getElementById("hidden-jilu-id").value = Personnel[index].id;
//                     return false;
//                 } else if (str == 'shenKe') {
//                     document.getElementById("input-lingdaoshenke").value = Personnel[index].name;
//                     document.getElementById("hidden-lingdaoshenke-id").value = Personnel[index].id;
//                     return false;
//                 } else {
//                     return true;
//                 }
