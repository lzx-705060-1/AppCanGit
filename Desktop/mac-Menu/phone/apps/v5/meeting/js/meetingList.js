...
cmp.ready(function () {
	...
	setPageTitle("meeting.page.lable.meeting");
...
function initPageTab(result) {
	...
	} else {//没有会议的权限，跳转到会议室页面
	    cmp.event.trigger("beforepageredirect", document);
	    cmp.href.next(_meetingPath + "/html/meetingRoomAdminApproveList.html"+meetingBuildVersion);
	}
	if(result.haveMeetingRoomPerm || result.haveMeetingRoomApp){
	    _$("#meetingRoomEntrance").style.display = "";
	    if(result.haveMeetingRoomApp) {
            _$("#meetingApplyDiv").style.display = "";
            _$("#addKey").style.display = "";
        }
	}
...
function initEvent(){
	...
	_$("#meetingRoomEntrance").addEventListener("tap", function(){
	    cmp.event.trigger("beforepageredirect", document);
        cmp.href.next(_meetingPath + "/html/meetingRoomAdminApproveList.html"+meetingBuildVersion,{},{animated:false});
    });
    //快捷键
	...
	document.addEventListener("meeting.ListRefresh", function(ret){
		var closePage = ret.data.closePage;//关闭弹层
    	var refreshList = ret.data.refreshList;//刷新列表数据
    	var listType = ret.data.listType;//页签
    	var goMeetingRoom = ret.data.goMeetingRoom;//是否跳转至会议室页签
    	
    	if(closePage && closePage == "true"){
    		fnClose();
    	}
    	if(refreshList && refreshList == "true"){
    		...
    		if(listType){
    			pageX.listType = listType;
    			_currentListDiv = "#"+getOpenFrom()+"Contain";
    			newTabContain(function(){
    				if(pageX.listViewKey[_currentListDiv]){
    					cmp.listView(_currentListDiv).destroyListview();
    				}
    				
    				loadData(tab2DataContainer[pageX.listType]["container"],[{},pageX.cache],tab2DataContainer[pageX.listType]["fn"]);				
    			});
    		}else{
    			cmp.listView(_currentListDiv).pullupLoading(pageX[_currentListDiv]['pageNo']);
    		}
    	}
    	if(goMeetingRoom && goMeetingRoom == "true"){
			cmp.href.next(_meetingPath + "/html/meetingRoomAdminApproveList.html"+meetingBuildVersion);
    	}
	});
}
...
function batchEventFn(){
  ...
  _$(".meet_room").style.display="none";
