...
function _goBack() {
    ...
    if(cmp.href.getParam() && cmp.href.getParam().openFrom == "sent"){
        btns = [{
            key : "delete",
            name : cmp.i18n("meeting.page.lable.cancelCreat")//放弃新建
        }];
    } else {
        btns = [{
            key : "saveDraft",
...
function showOrHideMore(){
	if(pageX.cache.showMore){
		...
		_$("#showOrHideMore").innerHTML = cmp.i18n("meeting.meetingCreate.hide")+"<span class='see-icon-arrow-up icon'></span>";
	}else{
		_$("#moreItme").style.display = "none";
		_$("#showOrHideMore").innerHTML = cmp.i18n("meeting.meetingCreate.show")+"<span class='see-icon-arrow-down icon'></span>";
	}
...
function showPlace(check){
	...
	if(typeof(pageX.haveMeetingRoomApp) == "undefined" && (typeof(urlParam) == "undefined" || typeof(urlParam["haveMeetingRoomApp"])=="undefined")){
		getUserPeivMenu(check, urlParam);
	}else{
		pageX.haveMeetingRoomApp = typeof(pageX.haveMeetingRoomApp) == "undefined" ? urlParam["haveMeetingRoomApp"] : pageX.haveMeetingRoomApp;
		_showPlace(check, urlParam);
	}
}

function _showPlace(check, urlParam){
	if(check && !_$("#meetingPlace").getAttribute("readonly") && !_$("#meetingPlace").value){
		return;
	}
	var userPicker = new cmp.PopPicker();
	if (pageX.haveMeetingRoomApp) {
		userPicker.setData([{
		    value : "1",  //已申请
...
function showType(check){
	...
	if(pageX.action = "modify" && pageX.cache.meetingType && pageX.cache.meetingType.isFormTrigger && "1" == pageX.cache.meetingType.isFormTrigger){
		var dataArray = new Array();
		 ...
		 dataArray.push({value : pageX.cache.meetingType.meetingType_value,  text : "表单触发会议"});
...
function callSeeyonAttachment(){
	var initParam = {
		...
		uploadId : "picture",
		handler : "#attBtn",
		initAttData : pageX.cache.attachments,
...
function showAttCount(attCount, assCount){
	var tempHtml = cmp.i18n("meeting.meetingCreate.attachment") + "(" + attCount + ")" + cmp.i18n("meeting.meetingCreate.and") + 
