...
cmp.ready(function () {
	...
	setPageTitle("meeting.page.lable.meetingDetail");
    ...
    $s.Meeting.detail(paramData, {
		success : function(result) {
			...
			LazyUtil.addLoadedFn("lazy_cmp", function(){
	            ...
	            //会议没有结束且存在回执状态就显示
	            if(page.feedbackFlag != "null" &&  page.meetingIsEnd == "0"){
				  showReceiptInfo();
...
function initPageDom(){
	...
	var meetingContent = document.querySelector(".meeting-detail-content");
	var buttonArea = document.querySelector("#buttonArea");
	
	var buttonAreaH = !buttonArea ? 0 : buttonArea.offsetHeight;
	meetingContent.style.height = (windowH - buttonAreaH) + 'px';
}
...
function initBtnView() {
	...
	if(page.canSummary && page.meeting.recordId  != "0"){
		...
		_$("#showSummaryLi").style.display = "";
	}

	if(page.canSeeTask && page.canNewTask && !page.isImpart){
		_$("#showTask").style.display = "";
		_$("#showTaskLi").style.display = "";
	}
	...
	//邀请后面还有其他按钮的时候显示分割线
	if(page.meeting.canInvite && (page.canReply || page.canQuickReply)) {
...
function _invitePer(msg){
    ...
    _params = {
            "meetingId" : meetingId,
            "beginDatetime" :new Date(beginDate.replace(/\-/g, "/")).getTime(),
            "endDatetime" : new Date(endDate.replace(/\-/g, "/")).getTime(),
            "emceeId" : null,
            "recorderId" : null,
            "conferees" : conferees,
...
function gotoVideo(){
	...
	if (typeof(page.video_url)!="undefined") {
		if (page.meeting.emceeId == page.currentUserId) {//主持人起会
			params = {
					"meetingId" : page.meeting.id,
...
function loadVideoInBrowse() {
	...
	cmp.sdk.loadAppInBrowse(option, 
			function(){}, //成功回调
...
function mettingReplyinit(){
	...
	_initParamData_r();
	setPageTitle("meeting.page.action.receipt");
	_initPageBack();
...
function _storePageObj(){
	pageX.cacheContent = _$("#content").value;
...
function _initEvent() {    
	...
	_$("#animateR .shadebg").addEventListener("tap",function(){
		 _goBackR();
	});
...
//回执
//监听软键盘弹起
cmp('body').on('focusin','#content',delayHideOrShowAttForm);
//监听软键盘收起
cmp('body').on('focusout','#content',delayHideOrShowAttForm);
...
function calHeight(){
	...
	_$("#content").style.height = winH - footerH -headerH - navH - winH*0.2 +"px";
}
function showHeight(){
    //震荡回复及知会节点 隐藏回复态度 --- 回执
    if(page.operate != "comment" && page.meeting.businessType != 9) {
...
function CshowHeight(){
	...
	_$("#animateR").style.bottom = "0px";
	// alert(winH - footerH -headerH - navH - winH*0.2);
	_$("#content").style.height = winH - footerH -headerH - winH*0.2 +"px";
}
//关闭软键盘计算页面高度  --- 撤销
function CshowHeight(){
...
function mettingCancelinit(){
	...
	ClassList.add('cmp-active');
	_initPageBackC();
	setPageTitle("meeting.meetingRoom.calcelPostscript");
