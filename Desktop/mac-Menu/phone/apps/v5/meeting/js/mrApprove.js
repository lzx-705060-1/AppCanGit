...
function initPageTitle() {
	if ("mrApproveList" == urlParam.openFrom) {
		//会议室申请
		setPageTitle("meeting.page.lable.title.room.approve");
	} else {
		//会议室审核
		setPageTitle("meeting.meetingRoom.mrApprove");
	}
}
			...
			if(result){
				...
				if(urlParam.openType == "todo"){
					...
					if(MeetingUtils.isFromM3NavBar()){
						...
						cmp.href.go(_meetingPath + "/html/meetingRoomAdminApproveList.html"+meetingBuildVersion);
					}
				}
			}
		},
