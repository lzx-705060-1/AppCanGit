(function ($) {
    ...
    function Scroller(elem, settings) {
        ...
        function generateWheelItems(i, type, year, month, date, hour, minute) {
            ...
            for (j in warr[i]) {
                ...
                html += '<div class="dw-li dw-v" data-val="' + j + '" style="height:' + hi + 'px;line-height:' + hi + 'px;"><div class="dw-i"';
                if (type == "date") {
                    html += 'style="font-size: 1.625em;';
                } else {
                    html += 'style="font-size: 0.875em;';
                }

                if ((i == 0 && warr[i][j] == year) || (i == 1 && warr[i][j] == month) || (i == 2 && warr[i][j] == date)
                    || (i == 3 && warr[i][j] == hour) || (i == 4 && warr[i][j] == minute)) {
                    html += 'color:blue;';
                }

                if(i == 1 && s.lang == "en") {//对于英文版的，需要将当前月份的颜色进行改变处理
                    var monthStrObj = $(warr[i][j]);
                    var monthName = monthStrObj.html();
                    if(((s.monthNames.indexOf(monthName) + 1) + "") == month.replace("0","")){
                        html += 'color:blue;';
                    }
                }
                ...
                if (type == "datetime") {
                    switch (i) {
                        case 0 :
                            if(s.lang == "zh") {
                                displayText = warr[i][j] + s.yearText;
                            }else {
                                displayText = warr[i][j];
                            }
                            break;
                        case 1 :
                            displayText = s.monthNamesShort[j];
                            break;
                        case 2 :
                            if(s.lang == "zh") {
                                displayText = warr[i][j] + s.dayText;
                            }else {
                                displayText = warr[i][j];
                            }
                            break;
                        case 3 :
                            if(s.lang == "zh") {
                                displayText = warr[i][j] + s.hourText;
                            }else {
                                displayText = warr[i][j] + "h";
                            }
                            break;
                        case 4 :
                            if(s.lang == "zh") {
                                displayText = warr[i][j] + s.minuteText;
                            }else {
                                displayText = warr[i][j] + "m";
                            }
                            break;
                        default :
                            break;
        ...
        function read() {
            var val = that.cacheVal;
            that.temp = ((input && that.val !== null && that.val != val) || that.values === null) ? s.parseValue(val || '', that) : that.values.slice(0);
            that.setValue(true);
        }
        ...
        function position(check,e) {
            ...
            if(s.zoomPage) {
//                s.currentLeft = document.body.scrollLeft;
//                s.currentTop = document.body.scrollTop;
                s.currentLeft = e.pageX - 150;
                s.currentTop = e.pageY - 120;
            }
            css.left = l;
            d.css(css);
            ...
            if(s.zoomPage) {
                $('.dw-persp',dw).css("height",$(window).height());
                $('.dw-widget',dw).addClass("dw-show");
            }else {
        ...
        that.change = function (manual) {
            ...
            if (s.display == 'inline') {
                ...
                var title = (s.title.length > 0) ? s.title : ((s.preset == 'date') ? s.dateChooseText : s.datetimeChooseText);
                $('.dwv', dw).html(title);
            }
         ...
         */
        that.show = function (prevAnim,e) {
            if (s.openCallback && typeof s.openCallback != "undefined") {
                s.openCallback();//添加打开的回调函数
            }
            var documentWidth = $(window).width();
            if(s.zoomPage) {
                if( $("meta.forbid").length == 0) {
                    var forbidScaleLable = "<meta class='forbid' name='viewport'"+
                        " content='width=device-width,initial-scale=1,user-scalable=no'/>";
                    $("head").append($(forbidScaleLable));
                    if($("meta.can").length > 0) {
                        $("meta.can").remove();
                    }
            ...
            setTimeout(function(){
                ...
                var type = s.preset;//组件类型：“date”：日期控件；“datetime”：日期+时间控件
                ...
                var dwWidgetClassStr = s.zoomPage?"dw-widget":"";
                var dwbg2ClassStr = s.zoomPage?"dwbg2":"";
                var html = '<div class="dw-trans dw-main ' + s.theme + ' dw-' + s.display + '">' + (s.display == 'inline' ? '<div class="dw dwbg dwi '+dwbg2ClassStr+'"><div class="dwwr">' : '<div class="dw-persp">' + '<div class="dwo"></div><div class="dw dwbg ' + mAnim + ' '+dwbg2ClassStr+'"><div class="dw-arrw"><div class="dw-arrw-i"><div class="dw-arr"></div></div></div><div class="'+dwWidgetClassStr+'"><div class="dwwr">' + (s.headerText ? '<div class="dwv"></div>' : ''));
                html += '<div class="dwc' + (s.mode != 'scroller' ? ' dwpm' : ' dwsc') + (s.showLabel ? '' : ' dwhl') + '"><div class="dwwc dwrc"><div class="dw-area"><ul>';
                for (i = 0; i < s.wheels.length; i++) {
                    // Create wheels
                    for (label in s.wheels[i]) {
                        warr[l] = s.wheels[i][label];
                        html += '<li><div class="dwwl dwrc dwwl' + l + '">' + (s.mode != 'scroller' ? '<div class="dwwb dwwbp" style="height:' + hi + 'px;line-height:' + hi + 'px;"><span>+</span></div><div class="dwwb dwwbm" style="height:' + hi + 'px;line-height:' + hi + 'px;"><span>&ndash;</span></div>' : '') + '<div class="dwl">' + label + '</div><div class="dww" style="height:' + (s.rows * hi) + 'px;"><div class="dw-ul">';
                        // Create wheel values
                        html += generateWheelItems(l, type, year, month, date, hour, minute);
                        html += '</div><div class="dwwo"></div></div><div class="dwwol"></div></div></li>';
                        l++;
                    }

                }
                html += '</ul></div></div></div></div>';
                html += (s.display != 'inline' ? '<div class="dwbc"><span class="dwbw dwb-c" ><span class="dwb" style="border-radius:0px;">' + s.cancelText + '</span></span>' + '<span class="dwbw dwb-n" ><span class="dwb">' + s.clearText + '</span></span>' + '<span class="dwbw dwb-s" ><span class="dwb">' + s.setText + '</span></span></div></div>' : '<div class="dwcc"></div>') + '</div></div></div>';
                dw = $(html);
                    ...
                    that.tap($('.dwb-s span', dw), function () {
                        if (that.hide(false, 'set') !== false) {
                            ...
                            if($("meta.forbid").length > 0) {
                                ...
                                $("meta.forbid").remove();
                                if($("meta.can").length == 0) {
                                    var canScaleLable = "<meta class='can' name='viewport'"+
                                        " content='width="+documentWidth+",initial-scale=1,user-scalable=yes'/>";
                                    $("head").append($(canScaleLable));
                                }
                                window.scrollTo(s.currentLeft, s.currentTop);

                            }
                        }
                        if (s.okCallback && typeof s.okCallback != "undefined") {
                            s.okCallback(s.formatResult(that.temp));//添加确定的回调函数（把值抽取出来,用于回调处理）
                            that.cacheVal = s.formatResult(that.temp);
                        }
                        that.cancel();
    ...
    var scrollers = {},
        ...
        defaults = {
            ...
            dateChooseText: 'Choose date',//添加一个“选择日期”的英文文字
            datetimeChooseText: 'Choose date time',//添加一个“选择日期时间”的英文文字
            scrollLock: true,
            tap: true,
            title: "",//添加一个组件标题的属性
            initVal:"",//添加一个初始化值的属性
            zoomPage : false,//添加一个是否是可缩放页面的属性
            okCallback: undefined,//添加一个确定的回调函数
            cancelCallback: undefined,//添加一个取消的回调函数
            clearCallback: undefined,//添加一个不设置按钮的回调
            openCallback:undefined,//添加一个打开组件的回调
    ...
    $.mobiscroll.themes.ios = {
        defaults: {
            dateOrder: 'yyMMd',//改变英文版日历显示排序规则（原来是MMdyy--------->yyMMd）
            rows: 5,
            height: 30,
            width: 55,
            headerText: false,
