;(function ($, window, iSP, undefined) {
	iSP._showDetail = function ($obj){
		...
		//var documentId = $obj.attr("documentId");
		...
		that.getSignInfo({signatures:_signature,callback:that._showDetailCallBack,dialog:that.dialog.config.id});
		iSP.winopenEvent();
	};
	...
	iSP._delSignature = function (action){
		...
		_ps.keyPassword = kgId("kg_pwdInput").value;
		dialog.hide();
		that.loadingwin();
	...
	iSP.templates = {
	   ...
	   delSignatureHTML : '<div id="kg_delSignature" class="kg_signature" >'+
	   				'<table class="kg_form">'+
						...
						'<tr>'+
							'<td class="kg_field">\u5bc6\u7801\uff1a</td>'+
							'<td id="kg_pwd" class="kg_value"><input type="password" maxlength="20" name="kg_pwdInput" id="kg_pwdInput" /> '+
							'</td>'+
						'</tr>'+
					'</table>'+
					'<div class="kg_buttons">'+
