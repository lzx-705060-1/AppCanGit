...
//	        });
          ...
          errorArray.push(curCheckObj.errorArray);
        }
	}
}
  function MxtCheckForm(form, options) {
	...
	if(options && options.validateHidden){
		...
		//TODO  修改客户bug   author hejianliang 
			var  inputType = this.tagName.toLocaleLowerCase();
          if ((inputType=='select' ||!this.disabled) && $(this).attr("validate") && this.tagName.toLowerCase()!="span" && this.tagName.toLowerCase()!="div")
            result.push($(this));
        });
	} else {
	//TODO 这个情况暂时不做处理
	...
	if(photoValidateCache &&  photoValidateCache instanceof Array){
		for(var x = 0 ; x <photoValidateCache.length;x++ ){
			result.push(photoValidateCache[x]);
		}
...
var temp = checkInput(obj, true);
  ...
  function getCheckObj(input) {
    ...
    if (input.data("checkObj")) {
      ...
      if(input.val() =="") {
  	    var relationL = $(parent).find("span[onclick ^='showRelationList']");
  	    if(relationL && relationL.length >0) {
  	    	var eqrelation = relationL.eq(0);
  	    	if(eqrelation) {
  	    		checkObj.value = input.text();
  	    	}
  	    }
      }
    ...
    if (obj[notNull] || obj[nullable]) {
      if (isNull(obj.value)) {
        addErrorMessage(obj, obj.name
            + $.i18n('validate.notNull.js'), true,2);
        return false;
      }
    }
    return true;
  ...
  function checkNotNullWithourTrim(obj) {
    ...
    if (obj[notNullWithoutTrim]) {
      if (isNull(obj.value, true)) {
        addErrorMessage(obj, obj.name + $.i18n('validate.notNull.js'), true,2);
        return false;
      }
    }
    return true;
  ...
  function checkIsDate(obj) {
    ...
    if (obj[fieldType] == 3) {
      ...
      if (dateReg.test(value) != true) {
        addErrorMessage(obj, obj.name
            + $.i18n('validate.validate.yyyy.MM.dd.js'));
        return false;
      }
    }
    return true;
   ...
   * 添加错误信息
   */
  function addErrorMessage(obj, errMsg, useSystem, type) {
    if (obj) {
      ...
      if (useSystem!=true && customErrorMsg) {
          result.errorMsg = customErrorMsg;
          obj.errorArray.push(result);
        } else {
