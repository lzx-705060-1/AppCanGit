...
//                    zcdbErrorArray.push(errorMsg);
    ...
    htmlSignatureM1 = function() {
        ...
        if(typeof(params)=="undefined"){
            signatureKey ='m3_v5_cmp_form_signature_' + form.moduleId + '_' + form.rightId;
        }else{
            signatureKey ='m3_v5_cmp_form_signature_' + form.moduleId + '_' + form.rightId;
        }
        ...
        $("input[comp*='htmlSignature']").each(function(){
            var _value = $(this).val();
            if(cmp.platform.CMPShell){
                cmp.ajax({
                    url: cmp.serverIp + '/seeyon/rest/signet/findsignets/' + _value.substring(_value.indexOf("_")+1) + '/' + _value,
                    type: 'GET',
                    timeout: '30000',
                    dataType: 'json',
                    headers: {
                ...
                var imgPath=cmp.serverIp + "/seeyon/signatPicController.do?method=writeGIF&RECORDID="+
                _value.substring(_value.indexOf("_")+1)+"&FIELDNAME="+_value;
                var imgArea = $("<img/>").attr("height", "auto").attr("src", imgPath).width($("#"+this.id+"_span").width()-5);
                $(this).after(imgArea);
                $(this).closest("td").css("background-color","");
            }
