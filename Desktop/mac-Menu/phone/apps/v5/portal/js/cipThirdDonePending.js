	...
	cmp.listView("#allPending", {
        config: {
            ...
            dataFunc: function(params, options) {
            	getThirdPendingData(currentAppId, options, params);
            },
...
function getThirdPendingData (currentAppId, options, params) {
    var CMP_V5_TOKEN = window.localStorage.CMP_V5_TOKEN;
    var pendingURL = "/seeyon/rest/m3/pending/thirdDonePending?appId=" + currentAppId + "&pageSize=20&pageNo="+params["pageNo"];
	var _spaceUrl = replaceAjaxUrl(pendingURL);
	cmp.ajax({
        type: "GET",
        data: null,
        url: _spaceUrl,
