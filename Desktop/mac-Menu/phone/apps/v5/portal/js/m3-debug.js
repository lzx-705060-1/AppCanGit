...
;(function(global, factory) {
    global.debugModule = global.debugModule?global.debugModule:false;
    //初始化工厂
    global.m3Debug = factory();
    //seajs模块兼容
    if (global.define && global.module) {
        //定义模块
        define(function(require, exports, module){
            global.module.exports = global.m3Debug;
        });
    }
})(this, function() {
    //构造函数
    ...
    function initDom() {
        ...
        cssNode.href = '/seeyon/m3/apps/v5/portal/css/debug.css';
        //加载元素
        document.head.appendChild(cssNode);
        debugNode.id = 'm3-debug';
        debugPageNode.id = 'debug-page';
        debugNode.className = 'm3-debug';
        debugPageNode.className = 'debug-page display-none';
     ...
     *  @param 需要输出的值
     */
    function fireDebug(value) {
        if (!this.modelState) return;
        var e = document.createEvent('HTMLEvents');
        e.initEvent('m3-debug', true, true);
        e.data = value;
    ...
    function addDebugListener() {
        this._on(document, 'm3-debug', function(e) {
            console.log(e.data);
        });
    }
    ...
    function initEvent() {
        ...
        objDebug.addEventListener('touchend', function() {
            ...
            if (new Date().getTime() - xy.time <= 350) {
                //显示调试页面
                removeClass(document.getElementById('debug-page'), 'display-none');
                //隐藏调试按钮
        ...
        document.getElementById('db-close').onclick = function() {
            //显示调试页面
            addClass(document.getElementById('debug-page'), 'display-none');
            //隐藏调试按钮
            removeClass(objDebug, 'display-none');
        }
    ...
    function reWriteNativeFunction() {
        ...
        _this.console.log = function(arg) {
            console.info(arg);
     ...
     */
    function render(data, id, operExtend) {
        var str = '',
            className = isError(data) ? 'debug-red' : '',
            type;
        if (operExtend) {
            var hisId;
            for(var i in data) {
                ...
                if (isObjectCanExtend(data[i])) {
                    ...
                    str +=  "<div class='no-extend' id='" + hisId + "'>\
            ...
            if (isObjectCanExtend(data)) {
                ...
                str +=  "<div class='no-extend' id='" + tgtId + "'>\
                            " + dataToString(data) + "\
                        </div>";
            }
        }
    ...
    function isObjectCanExtend(obj) {
        ...
        //非null，且可枚举
        if (isObject(obj) && !isNull(obj) || isEvent(obj))
            return true;
        else
            return false;
    }
    ...
    /* -------------------------------- end --------------------------------- */
    return new debug(this.debugModule);
});