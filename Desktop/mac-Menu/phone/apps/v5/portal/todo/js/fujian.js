...
function getFujian() {
    ...
    url = url + "&username=" + loginname + "&pageNo=1&PHONE_ID=" + phoneid + "&affairid=" + affairid + "&summaryid=" + summaryid;
    console.log(url);
    obj.url=url;
...
function httpsucess(info_list) {
    ...
    function httpopensucess(data){
        ...
        var fileType = cmp.att.type(filename)
        if(imgFormat.indexOf(fileType.toLocaleLowerCase()) != -1){
            filename = filename+"1";
        }
        cmp.att.read({
