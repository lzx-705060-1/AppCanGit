...
(function() {
    ...
    function initData(tip) {
        ...
        getSortData(function(resp) {
			...
			for(var oi=0;oi<data.length;oi++){
				 if(data[oi].classificationName=="待办工作"||data[oi].classificationName=="全部待办"){
					 data[oi].classificationName="新OA待办";
					daibanData=data[oi]
				}
			}
            ...
            if(!olduser||!oldpwd){
                //门户接口获取ad信息为空，查询微协同数据库,如果也不存在则进入绑定页面存入ad信息到数据库
                cmp.ajax({
                    type: "get" ,
                    data: "",
                    url : cmp.seeyonbasepath+'/cost/formNCController.do?method=userInfo' ,
                    //url : 'https://oamobile.agile.com.cn/olduser/getuserbyname?olduser='+cmp.storage.get("ygzz_loginname") ,
                    async : false,
                    dataType : "json",
                    success : function ( r, textStatus, jqXHR ){
                        if(r&&JSON.stringify(r) != "{}"){
                            cmp.storage.save("olduser",r.olduser)
                            cmp.storage.save("oldpwd",r.oldpwd)
                            olduser=r.olduser
                            oldpwd=r.oldpwd
                        }
                    },
        ...
        /*cmp.storageDB.get('oldInfo',function(result){
            if(result.success){
                var data = result.data;//取出数据
                if(data&&data!=""){
                    olduser=data.split("=")[0]
                    oldpwd=data.split("=")[1]
                    //alert(olduser+"--"+oldpwd)
    ...
    function initEvent() {
        ...
        $(".toDo_listes").on("tap", "li", function() {
			if($(this).hasClass("appid-1")){
                ...
                info_mode.node_policy = title.substring(title.indexOf("[")+1,title.indexOf("]"));
                ...
                var url = serviceUrl + "/servlet/PublicServiceServlet?message_id=newdblist35_content";
                url = url + "&type=getcontent&url="+encodeurl ;
                obj.url=url
                        ...
                        $.ajax({
                            ...
                            success: function (cbData) {
                                ...
                                if (undefined != successFun && '' != successFun.trim()) {
                                    info_mode.affair_id = cbData.encodeurl.AFFAIRID;
                                    info_mode.summaryid = cbData.content.SUMMARYID;
                                    openDetail(info_mode)
    ...
    function setlistView(appId, isThird) {
        ...
        if(appId=="-1"){
            ...
            if(!olduser||!oldpwd){
                cmp.notification.alert("请先绑定AD帐号",function(){
                            cmp.href.next("old_user.html")
		...
		/*if(appId=="-1"){
            var olduser=""
            var oldpwd=""
            var oldInfo = cmp.storageDB.get('oldInfo',function(result){
                if(result.success){
                    //获取数据成功
                    var data = result.data;//取出数据
                    if(data&&data!=""){
                        ...
                        try {
                            ...
                            $.ajax({
                                ...
                                success: function (cbData) {
                                    ...
                                    console.log(cbData);
                                    if (undefined != successFun && '' != successFun.trim()) {
                                        if(cbData.stationguid){
                                            var stationguid = cbData.stationguid.GW.guid;
                                            if (stationguid) {
    ...
    //获取南宁数量
    function getOldDaiban3(param,option){
        //var _this=this
        var obj = new Object()
        var serviceUrl = cmp.storage.get('ygzz_serviceUrl');
        var phoneid = cmp.storage.get('ygzz_phoneid');
	...
	//获取3.5待办数据
            function getOldDaiban2(param,option){
				...
				var banlidata;
                // var templeteids =cmp.storage.get('templeteids');
                var url = serviceUrl + "/servlet/PublicServiceServlet?message_id=newdblist35";
                ...
                //cmp.dialog.loading("加载中...")
                    ...
                    if ('' != String(url).trim()) {
                        try {
                            ...
                            $.ajax({
                                url: "https://oamobile.agile.com.cn/oa2/pending?firstnum=1&pagesize=20&code="+loginname,
                                type: type,
                                data: data,
                                dataType: dataType,
                                contentType: 'application/json;charset=utf-8',
                                ...
                                success: function (cbData) {
									...
									//cmp.storage.save("ygzz_serviceUrl","https://mportal.agile.com.cn:8443");
                                    cmp.storage.save("serviceUrl","http://10.1.9.144")
                                    //cmp.storage.save('ygzz_org_department_id',cbData.user.org_department_id)
                                    //cmp.storage.save("ygzz_org_account_id",cbData.user.org_account_id);
                                    //cmp.storage.save("ygzz_org_account_id","608696710712280500");
                                    //cmp.storage.save("ygzz_superior",cbData.user.superior);
                                    if (undefined != successFun && '' != successFun.trim()) {
                                        //var reg = RegExp(/工号【(.*?)】不存在/)
                                        var dbObj = cbData
                                        if(!dbObj.list){
                                            banlidata=[]
                                        }else{
                                            banlidata=dbObj.list
                                        }
                                        if(cbData.formhide){
                                            formhide = cbData['formhide']
                                        }
	...
	function renderOldData(res,isRefresh){
		 //var _this=this
                ...
                for ( var i = 0; i < info_list_mode.length; i++) {
	                    html+='<li class="appid-2" data-index="'+i+'">'
						html+='<div class="content">'
						html+='<div class="info read">'
				...
				$(table).html(html)
        //颜色斑马线
        $('.toDo_listes li').each(function(index, item) {
...
console.log(obj)
 var extData={
        ...
        cmp.storage.save("link_title",obj.title)
