...
(function() {
    ...
    function initPage() {
        cmp.ready(function() {
            ...
            searchValue = cmp.href.getParam().data;
            initStyle();
            initEvent();
        });
    }
    ...
    function renderData(searchValue,pageSize,pageNo) {
        cmp.ajax({
            ...
            url: serverAdress + '/seeyon/rest/uc/rong/groups/searchgroup',
    ...
    function jumpToGroupChatting(){
        $("#uc-search-member").on("tap","li",function(){
            var id = this.getAttribute("data-id");
            var title = this.querySelector('#n_'+id).innerText;
            cmp.chat.start({
                title: title,
                id: id,
                type: "group",
