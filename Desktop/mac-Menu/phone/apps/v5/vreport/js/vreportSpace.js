(function(){
	...
	var initDom = function(){
		...
		var vreportReadList = document.querySelector("#vreportSpaceList");
	...
	var initData = function(){
		//渲染列表
		renderList(vreportCommon.getSpceList());
		//渲染上一次的页面状态
		renderPageState();
	}
	var renderList = function(data){
		...
		var listContainer = document.querySelector("#vreportSpaceList ul");
		listContainer.innerHTML = tpl;
		if(!scroller){
			scroller = new cmp.iScroll('#vreportSpaceList', {hScroll: false, vScroll: true,useTransition:true});	
		}else{
			scroller.refresh();
		}
	}
	...
	var events = function(){
		 ...
		 * 所有报表的关注
		 */
		cmp("#vreportSpaceList").on("tap",".btn-favour",function(){
			var self = this;
		 ...
		 * 所有报表的取消关注
		 */
		cmp("#vreportSpaceList").on("tap",".btn-cancel-favour",function(){
			var self = this;
		...
		//点击穿透
		cmp("#vreportSpaceList").on("click","li.cmp-table-view-cell",function(){
