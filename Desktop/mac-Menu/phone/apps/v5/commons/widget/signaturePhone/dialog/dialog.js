...
artDialog.fn = artDialog.prototype = {
	...
	_init: function (config) {
		...
		//添加属性 iehyou
		$(DOM.outer).attr("id",that.config.id );
		// 操作元素
	...
	content: function (msg) {
		...
		//wrap.style.width = 'auto'; iehyou
		if(that.config.width.indexOf("px")!=-1){wrap.style.width = that.config.width;}
	...
	position: function (left, top) {
		...
		if(ow == 0){
			ow = config.width.substring(0,config.width.length-2)
		}
	...
	lock: function () {
		...
		var that = this,
			...
			lockMaskWrap = that._lockMaskWrap || $(document.getElementsByTagName("body")[0].appendChild(document.createElement('div'))),
...
_path = window['_artDialog_path'] || (function (script, i, me) {
	for (i in script) {
		...
		if (script[i].src && script[i].id == 'kg_artDialog') me = script[i];
	};
				...
				'<td class="aui_c">' +
					...
					'<table class="aui_dialog">' +
						'<tbody>' +
							...
							'</tr>' +
							'<tr>' +
							'<td colspan="2" class="aui_td_footer">' +
								'<div class="aui_footer_wrap"><h1>\u79d1\u6280\u4f20\u627f\u53e4\u8001\u6587\u5316,\u5b89\u5168\u4fe1\u5b88\u4e00\u8bfa\u5343\u91d1</h1></div>' +
							'</td>' +
						'</tr>' +
						'</tbody>' +
					'</table>' +
