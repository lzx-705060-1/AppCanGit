        ...
        */
        ...
        this.DrawSticks = function ()
        {
            ...
            for (var i=0,len=this.angles.length; i<len; ++i) {
                ...
                co.lineWidth = 1;
                var labelsSL = prop['chart.labels.sticks.length'];
                var labelsSHL = prop['chart.labels.sticks.hlength'];
                if(i%2==0){labelsSL += 30;labelsSHL+=30;}
                
                points[0] = RG.getRadiusEndPoint(cx, cy, midpoint, radius + extra + offset);
                points[1] = RG.getRadiusEndPoint(cx, cy, midpoint, radius + labelsSL + extra - 5);
                
                points[2] = RG.getRadiusEndPoint(cx, cy, midpoint, radius + labelsSL + extra);
                
                points[3] = RG.getRadiusEndPoint(cx, cy, midpoint, radius + labelsSL + extra);
                points[3][0] += (points[3][0] > cx ? 5 : -5);

                points[4] = [
                             points[2][0] + (points[2][0] > cx ? 5 + labelsSHL : -5 - labelsSHL),
                             points[2][1]
                            ];
