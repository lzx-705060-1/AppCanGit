    ...
    */
    ...
    RGraph.HTML.Key = function (id, prop)
    {
        ...
        */
        ...
        var newStr = "<div id='rgraph_key' style='margin: 0;padding: 0;'><div id='rgraph_key_sub' style='margin: 0 auto;'>";

        /**
        * Add the individual key elements
        */
        for (var i=0; i<prop.labels.length; i+=1) {
            newStr += '<div class="rgraph_key_sub_item" style="float: left;height:22px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis; margin-right: 10px;">';
            newStr += '<div  style="display: inline-block; margin-right: 5px; margin-top: 4px;line-height: 14px; width: 14px; height: 14px; background-color: ' + prop.colors[i] + ';">&nbsp;</div>';
            newStr +='<span style="font-size: 14px;">'+prop.labels[i]+'</span>';
            newStr += '</div>';
...
//        div.innerHTML += (str + '</table>');
        div.innerHTML =  (newStr + '</div></div>') + div.innerHTML;

        var subItems = document.getElementById("rgraph_key_sub").childNodes;
        var rgrapgh_key_sub_width = 0;
        for(var i = 0;i<subItems.length;i++){
            rgrapgh_key_sub_width += subItems[i].offsetWidth + 10;
        }
        document.getElementById("rgraph_key_sub").style.width = rgrapgh_key_sub_width + "px";
        if(document.getElementById("rgraph_key_sub").offsetWidth >= document.getElementById("container").offsetWidth){
            document.getElementById("rgraph_key_sub").style.width = document.getElementById("container").offsetWidth + "px";
        }
