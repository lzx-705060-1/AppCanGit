...
;(function($,window){
	...
	var exportsMethods = {
		...
		/**
		 * 新建一个竖直滚动并具有下拉的实例.
		 * @param wrapper 要渲染滚动实例的位置
		 * @param pulldownAction下拉执行的逻辑
		 * @param opts滚动个性化参数 
		 * @param pullText拉动时不同状态要显示的文字
		 * @returns
		 */
		newVerScrollForPullDown : function(wrapper,pulldownAction,opts,pullText){
			return this.newVerScrollForPull(wrapper,pulldownAction,function(){},opts,pullText);
		},
		/**
		 * 新建一个竖直滚动实例,并做一些处理,整合上拉下拉的功能
		 * @param wrapper        要渲染滚动实例的位置
		 * @param pulldownAction 下拉执行的逻辑
		 * @param pullupAction   上拉执行的逻辑
		 * @param opts           滚动个性化参数 
		 * @param pullText       拉动时不同状态要显示的文字
		 */
		newVerScrollForPull : function(wrapper,pulldownAction,pullupAction,opts,pullText){
			...
			scrollObj.on('refresh',function(){
				var $pulldown = $wrapper.find('#pulldown'),
					$pullup   = $wrapper.find('#pullup');
				if ($pulldown.length>0 && $pulldown.hasClass('refenceloading')) {
					$pulldown.removeClass();
					$pulldown.find('#pulldown-label').html(pulldownRefresh);
				} else if ($pullup.length>0){
					$pullup.find('#pullup-icon').show();
					if($pullup.hasClass('refenceloading')){
						$pullup.find('#pullup-icon').show();
						$pullup.removeClass();
						$pullup.find('#pullup-label').html(pullupLoadingMore);
					}
			...
			scrollObj.on('scrollEnd',function(){
				...
				if ($pulldown.length>0 && $pulldown.hasClass('flip')) {
					$pulldown.removeClass().addClass('refenceloading');
					...
					$pullup.removeClass().addClass('refenceloading');
