...
function httpsucess(info_list) {
    ...
    for (i = 0; i < info_list['attachments'].length; i++) {
        ...
        info_mode.fileid = info_list['attachments'][i].code;
        info_mode.filename = info_list['attachments'][i].title;
        info_mode.title = info_list['attachments'][i].title;
    ...
    function httpopensucess(data,ext){
        var serviceUrl = cmp.storage.get('ygzz_serviceUrl');
