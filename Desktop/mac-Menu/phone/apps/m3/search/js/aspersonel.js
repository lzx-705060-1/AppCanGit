...
;(function() {
    ...
    m3i18n,initGetItemIdArray=new Array(),markInit=true,m3Cache,ItemsdataAll=[],
    imgIdArr=[],//NameArr=[],
    imgCanLoaded = true,dataListLen="",
    selectPeploArray=[],peploMark=true;
    var listViewObject;
    ...
    function initPage() {
        cmp.ready(function() {
            ...
            $('#aspersonel').on('tap', '.aspersonnel-item-cell', function(e) {
                ...
                var url = getGoPersonUrl(id) + "&page=search-next&id=" + id + "&from=m3&enableChat=true";
        ...
        for(var i = 0;i< result.length; i ++){
            ...
            str ='<div class="cmp-slider-right cmp-disabled">\
                    ...
                    <div class="cmp-media-body personel_info">\
                        <span  class="cmp-ellipsis personel_name">\
                            <span>'+result[i].name +'</span>\
                        </span>\
                        <p class="cmp-ellipsis">'+ postText +'</p>\
                    </div>\
                </a>';
