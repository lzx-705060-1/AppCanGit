...
//初始化key-表情样式
    EmojiConfig.emojiicons = function(){
        var icons = {}
        var dataItem;
        var row;
        var column;
        ...
        while(j < EmojiConfig.EmojiCategories.length){
            ...
            while(i < EmojiConfig.EmojiCategories[j].length){
                ...
                icons[':' + name + ':'] = [j, row, column, ':' + name + ':', EmojiConfig.EmojiCategories[j][i]];
                i++;
            }
            j++;
        }
...
//创建img对象
    EmojiConfig.createIcon = function(emoji){
        ...
        var assetsPath = '/seeyon/m3/apps/v5/show/img';
        var filename =  assetsPath+'/emoji_spritesheet_!.png';
        var blankGifPath =  assetsPath+'/blank.gif';
        var iconSize = 10;
    ...
    var  EmojiUtil =function(){
        var self=this;
//常用表情
        self.defaultiocns = ':joy:,:kissing_heart:,:heart:,:heart_eyes:,:blush:,:grin:,:+1:,:relaxed:,:pensive:,:smile:,:sob:,:kiss:,:unamused:,:flushed:,:stuck_out_tongue_winking_eye:,:see_no_evil:,:wink:,:smiley:,:cry:,:stuck_out_tongue_closed_eyes:,:scream:,:rage:,:smirk:,:disappointed:,:sweat_smile:,:kissing_closed_eyes:,:speak_no_evil:,:relieved:,:grinning:,:yum:,:laughing:,:ok_hand:,:neutral_face:,:confused:'
            .split(',');
    };


//将ios输入结果转换成标准的表情格式
    EmojiUtil.prototype.EmojiToString = function(content){
        var self=this;
        if(self.isEmojiCharacter(content)){
            //映射关系初始化
            EmojiConfig.init_unified();
            //初始化常用表情
            var defaultemoji = new Object();
            for(var i = 0;i<self.defaultiocns.length;i++){
                defaultemoji[self.defaultiocns[i]] = self.defaultiocns[i];
            }
            self.defaultemoji = defaultemoji;

            var emojiList=[];
            //匹配常用表情
            emojiList = self.collectEmoji(emojiList,content.match(new RegExp(/\uD83E[\uDD10-\uDDC0]|[\u00A9-\u3299]|\ud83d[\ude00-\ude4f]|\uD83C[\uDC04-\uDFF0]|[\u0023-\u0039]\u20E3|\uD83C[\uDDE8-\uDDFA]\uD83C[\uDDF3-\uDDF8]|\uD83E\uDD18\uD83C\uDFFB/g)));
            if(emojiList.length>0){
                for(var i=0;i<emojiList.length;i++){
                    if(EmojiConfig.reversemap[emojiList[i]]) {
                        content=content.replace(emojiList[i],":" + EmojiConfig.reversemap[emojiList[i]] + ":");
                    } else {
                        content=content.replace(emojiList[i] + '','\[' + searchI18N[lang].expression + '\]');
                    }
                }
            }

            //过滤掉非常用表情
            return self.FiterEmoji(content);
        }else{
            return content;
        }
    };
...
//将带表情的字符串转换成标准表情
    EmojiUtil.prototype.StringToEmoji = function(content){
        if(content != null){
            var emojis = content.match(new RegExp(":[0-9a-zA-Z_]*:","g"));
            if(emojis != null){
                for(var i = 0;i<emojis.length;i++){
                    var tempEmoji;
                    if(EmojiConfig.emojiicons[emojis[i]]) {
                        tempEmoji = EmojiConfig.emojiicons[emojis[i]][4];//todo 修改undifined的防护  xinpei
                    }
                    if(tempEmoji){
                        tempEmoji = EmojiConfig.emoji_data[tempEmoji][0][0];
                        content=content.replace(emojis[i],tempEmoji);
                    }
    ...
    var perch = function(content){
        var newStr = "";
        for (var i = 0; i < content.length; i++) {
            if (content.charCodeAt(i) > 255 || content.charCodeAt(i) < 0) {
                newStr+=content.substring(i,i+2)+"|";
                i ++;
            }else{
                newStr+=content.substring(i,i+1)+"|"}
        }
        return newStr;
    };
//过滤掉非常用表情
    EmojiUtil.prototype.FiterEmoji = function(content){
        var self=this;
        //先过滤掉已经匹配上的非常用表情格式
        var text = [],
            splitArr = content.split(/:[0-9a-zA-Z_]+:/),
            strArr;
        strArr = content.match(/:[0-9a-zA-Z_]+:/g) || [];
        for(var i = 0;i < splitArr.length;i++) {
            if(splitArr[i] === '' && strArr[i] !== undefined) { 
                text.push(strArr[i]);
            } else {
                var str = perch(splitArr[i]);
                str = str.replace(/\|/g,'');
                text.push(str);
                if(strArr[i])text.push(strArr[i]);
            }
        }
        var index = 0;
        content = '';
...
//获取emoji的位置
    EmojiUtil.prototype.getEmojiEndPos=function(text,startPos){
        var self=this;
        var best = -1;
        for (var j = startPos + 1; j <= text.length; j++) {
            if(self.isEmojiCharacter(text.slice(startPos,j).join(""))){
                best = j;
                break;
            }
        }
    ...
    EmojiUtil.prototype.collectEmoji=function(emojiList,emoticons){
        if(emoticons){
            emojiList = emojiList.concat(emoticons);
        }
        return emojiList;
    };
...
//判断是否是emoji表情
//判断是否是emoji表情
    EmojiUtil.prototype.isEmojiCharacter=function(substring){
        var self=this;
