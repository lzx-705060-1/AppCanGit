...
(function($){
    ...
    var IndexedList = $.Class.extend({
        ...
        caleLayout: function () {
            var self = this;
            var cmp_indexed_ul_extend=self.box.querySelector('.cmp-indexed-ul-extend');
            var CustomHeight=cmp_indexed_ul_extend?cmp_indexed_ul_extend.offsetHeight:0;
            var withoutSearchHeight = (self.box.offsetHeight - self.el.search.offsetHeight-CustomHeight) + 'px';
            self.el.bar.style.height = withoutSearchHeight;
            self.el.inner.style.height = withoutSearchHeight;
            var mark=40;
            if(cmp_indexed_ul_extend!=null){
                var lengthObj=cmp_indexed_ul_extend.querySelectorAll('li').length;
                if(lengthObj==1){
                    mark=140;
                }else if(lengthObj>=2){
                    mark=195;
            ...
            var barItemHeight = ((self.el.bar.offsetHeight - mark) / self.el.barItems.length) + 'px';
...
//mui(selector).indexedList 方式
    ...
    $.IndexedList = function (selector, options) {
        return $(selector).indexedList(options);
    }
})(cmp);