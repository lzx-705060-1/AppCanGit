...
(function($, window, document, undefined) {
    ...
    Picker.prototype.calcElementItemPostion = function(andGenerateItms) {
        ...
        self.elementItems.forEach(function(item) {
            ...
            // item.style.webkitTransform = "translate3d(0,0," + self.r + "px) rotateX(" + (-self.endAngle) + "deg)";
            if (andGenerateItms) {
                var dataItem = {};
    ...
    Picker.prototype.setAngle = function(angle) {
        var self = this;
        var moveY = -(angle/20 * 36);//将原来的旋转角度转换成y向值，适配ios11的坑----辛裴20180205
        self.list.angle = angle;
        // self.list.style.webkitTransform = "perspective(1000px) rotateY(0deg) rotateX(" + angle + "deg)";
        // self.list.style.webkitTransform = " rotate3d(0,1,0,0deg) rotateX(" + angle + "deg)";
        self.list.style.webkitTransform = 'translate3d(0,' + moveY+ 'px,0)';
    ...
    Picker.prototype.setItems = function(items) {
        ...
        self.items.forEach(function(item) {
            if (item !== null && item !== undefined) {
                var text = item.text?escapeScriptTag(item.text):item;
                buffer.push('<li value="'+(item.value || text)+'">' + text + '</li>');
            }
        });
    ...
    Picker.prototype.getSelectedIndex = function() {
        var self = this;
        var angle=self.list.angle / self.itemAngle;
        var fixedIndex=parseInt(angle.toFixed(0));
        if((fixedIndex >= self.items.length)){
            fixedIndex = self.items.length -1;
        }
        if(fixedIndex < 0) {
            fixedIndex = 0;
        }
